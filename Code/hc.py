...
class HC(sensors.Sensor):
    ...
    def get_delta_list(self):
        if sensors.file_exist(file_name):
            ...
            if len(self.delta_list) != 0:
                ...
                f = open(file_name, 'r')
                sub_functions.send_message(self.devices[0].ip, DataPackage.SAMPLES_MESSAGE, self.delta_list)
        ...
        while True:
            ...
            if sam is -1:
                ...
                if not self.is_in_range(5, sam):
                    pid = os.fork()
                    if pid is 0:
                        message = " get problem from " + socket.gethostname() + ": the measurement is:" + str(
    ...
    def send_in_fork(self, ip, pid, time_to_try, message):
        ...
        for i in range(time_to_try):
            try:
                res = sub_functions.send_message(ip,DataPackage.ERROR_MESSAGE, message)
                if res is not None:
                    is_transfer = True
                break
        ...
        if is_transfer is True:
			...
			if is_master is True:  # master has fallen and now this slave is the new master
				main.route()
